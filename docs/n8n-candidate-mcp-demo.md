...
| `403` from CloudFront, before any JSON body | `Host` header mismatch — n8n's HTTP Request node sets this automatically; only an issue if you're proxying the call through something that rewrites it.                                          |
